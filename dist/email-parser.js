...
exports.parseAnyEmail = parseAnyEmail;
exports.processEmailFile = processEmailFile;
exports.processEmailJson = processEmailJson;
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
// @ts-ignore - To ignore type issues with mailparser
const mailparser_1 = require("mailparser");
const cheerio = __importStar(require("cheerio"));
const iconv = __importStar(require("iconv-lite")); // Optional for character encoding
/**
...
async function parseStandardEmail(content, opts) {
    ...
    try {
        ...
        const parsed = await (0, mailparser_1.simpleParser)(processedContent, parseOptions);
...
function sanitizeHtml(html) {
    ...
    try {
        ...
        $('*').each(function () {
            var _a;
            const el = $(this);
            const attrs = (_a = el.get(0)) === null || _a === void 0 ? void 0 : _a.attribs;
            if (!attrs)
                return;
...
function extractAttachments(parsedEmail, opts) {
    ...
    if (opts && opts.debug) {
        ...
        if (parsedEmail.attachments) {
            parsedEmail.attachments.forEach((att, i) => {
                log(`  Raw attachment ${i + 1}:`, `filename=${att.filename || 'undefined'}`, `contentType=${att.contentType || 'undefined'}`, `contentDisposition=${att.contentDisposition || 'undefined'}`);
            });
        }
        else {
    ...
    if (parsedEmail.html) {
        try {
            ...
            $('a').each((i, elem) => {
                ...
                const isFileLink = href && (href.includes('sharepoint.com') ||
            ...
            $('img').each((i, elem) => {
                ...
                if (seenUrls.has(src) || src.startsWith('data:'))
                    return;
        ...
        if (attachment.iconUrl) {
            ...
            for (let j = 0; j < attachments.length; j++) {
                if (attachments[j].url === attachment.iconUrl) {
                    ...
                    (_a = attachments[j].isIconFor) === null || _a === void 0 ? void 0 : _a.push(i);
                    break;
                }
            }
        }
    ...
    if (opts && opts.debug) {
        ...
        attachments.forEach((att, i) => {
            log(`  Attachment ${i + 1}: ${att.filename}, type=${att.type}, ${att.isLink ? 'linked' : 'standard'}${att.isLikelyIcon ? ', ICON' : ''}${att.iconAttachmentIndex !== undefined ? `, icon at index ${att.iconAttachmentIndex}` : ''}${att.isIconFor ? `, is icon for attachments: ${att.isIconFor.join(', ')}` : ''}`);
        });
    }
    return attachments;
}
...
function determineFileType(filename, contentType, iconUrl) {
    ...
    if (filename) {
        ...
        if (lower.includes('.pdf'))
            return 'pdf';
        if (lower.match(/\.(docx?|odt|rtf)$/))
            return 'docx';
        if (lower.match(/\.(xlsx?|csv|ods)$/))
            return 'xlsx';
        if (lower.match(/\.(pptx?|odp)$/))
            return 'pptx';
        if (lower.match(/\.(txt|log|md)$/))
            return 'txt';
        if (lower.match(/\.(html?|xhtml)$/))
            return 'html';
        if (lower.match(/\.(zip|7z|rar|tar|gz)$/))
            return 'zip';
        if (lower.match(/\.(jpe?g|png|gif|bmp|tiff?|webp|svg)$/))
            return 'image';
        if (lower.match(/\.(mp4|mov|wmv|flv|avi|webm|mkv)$/))
            return 'video';
 ...
 */
function hasAttachmentsInPayload(payload) {
    if (!payload)
        return false;
